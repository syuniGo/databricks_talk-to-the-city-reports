...
class PDFProcessor:
    def __init__(self):
        ...
        self.store_path = os.getenv("store_path")
    ...
    def process_multiple_pdfs(self, urls: List[str], output_file: str):
        ...
        for url in urls:
            ...
            pdf_save_path = f"{self.store_path}/{pdf_filename}"
        ...
        if all_data:
            logging.info(f"save_to_csv to {self.store_path}/{output_file}")
            self.save_to_csv(all_data, f"{self.store_path}/{output_file}")
    ...
    def save_to_csv(self, data: List[Dict], output_file: str):
        logging.info(f"save data to {output_file}")
        try:
            logging.info(f"save data to {output_file}")
...
def main():
    ...
    processor.process_multiple_pdfs(pdf_urls, "aipdfs.csv")
